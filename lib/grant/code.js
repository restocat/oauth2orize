 ...
 *
 *     function({client, redirectURI, user, res, req, locals}, done) { ... }
 *
 ...
 * Examples:
 *
 *     server.grant(oauth2orize.grant.code(function({client, redirectURI, user, res}, done) {
 *       AuthorizationCode.create(client.id, redirectURI, user.id, res.scope, function(err, code) {
...
module.exports = function code(options, issue) {
  ...
  function response(txn, res, complete, next) {
    ...
    const options = {
      client: txn.client,
      redirectURI: txn.req.redirectURI,
      user: txn.user,
      res: txn.res,
      req: txn.req,
      locals: txn.locals
