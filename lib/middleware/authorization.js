 ...
 *
 *     function({clientID, redirectURI, scope, type, req}, done) { ... }
 *
...
module.exports = function(server, options, validate, immediate, complete) {
  ...
  options = options || {};
  immediate = immediate || function ({}, done) { return done(null, false); };
  ...
  return function authorization(req, res, next) {
    ...
    const body = req.body || {};
    const type = req.query.response_type || body.response_type;
    ...
    server._parse(type, req, function(err, areq) {
      ...
      function validated(err, client, redirectURI, webOrigin) {
        ...
        const params = {
          ...
          type: req.oauth2.req.type,
          req: req.oauth2.req,
          locals: req.oauth2.locals,
          txn: req.oauth2
        };

        immediate(params, immediated);
      }
