...
module.exports = function(server, options, parse, complete) {
  ...
  const cancelField = options.cancelField || 'cancel'
  ...
  return function decision(req, res, next) {
    ...
    parse(req, function(err, ares, locals) {
      ...
      if (req.oauth2.res.allow === undefined) {
        req.oauth2.res.allow = !req.body[cancelField];
      }
      ...
      res.end = function(chunk, encoding) {
        if (server._txnStore.legacy === true) {
          server._txnStore.remove(options, req, req.oauth2.transactionID, noop);
        } else {
          server._txnStore.remove(req, req.oauth2.transactionID, noop);
        }
